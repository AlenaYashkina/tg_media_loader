...
from .downloader import ProgressReporter
...
class GuiProgress(ProgressReporter):
    """Progress reporter that updates the GUI status and log area."""

    def __init__(self, update_status: Callable[[str, str], None]) -> None:
        self._update_status = update_status
        self._total: int | None = None
        self._processed = 0

    def history_started(self, total_messages: int | None) -> None:
    ...
    def media_downloaded(self) -> None:
        # No-op; GUI focuses on message progress.
        pass

    def download_started(self, label: str, total: int | None):
        # GUI hides per-file progress to keep noise down.
        return None
    ...
    def _update(self) -> None:
        if self._total:
            text = f"Скачивание: {self._processed} из ~{self._total} сообщений"
        else:
            text = f"Скачивание: {self._processed} сообщений"
        self._update_status(text, "blue")
...
async def _sign_in(
    ...
    try:
        try:
            ...
            if not password:
                raise
            # 2FA-enabled accounts require a second call with the password only.
...
def _enable_context_menu(widget: tk.Widget) -> None:
...
def _collect_chat_urls(chat_input: tk.Text) -> list[str]:
    raw_value = chat_input.get("1.0", tk.END)
...
def _execute_login_dialog(root: tk.Tk, config: AppConfig) -> bool:
    ...
    def _submit() -> None:
        ...
        try:
            ...
            status_label.configure(text="Incorrect 2FA password.", foreground="red")
        except Exception as exc:
...
def start_gui() -> None:
    ...
    ttk.Label(main_frame, text="Chat URLs (one per line):").grid(row=0, column=0, sticky="nw")
    chat_text = scrolledtext.ScrolledText(main_frame, height=4, wrap="none")
    _enable_context_menu(chat_text)
    chat_text.grid(row=0, column=1, columnspan=2, pady=4, sticky="ew")
    ...
    def _worker(
        ...
        progress = GuiProgress(_set_status)

        try:
            failures: list[str] = []
            total = len(chat_urls)
            for index, chat_url in enumerate(chat_urls, start=1):
                LOGGER.info("Starting download for chat %s (%s/%s)", chat_url, index, total)
                _set_status(f"Downloading {index}/{total}", "blue")
                try:
                    asyncio.run(
                        run_download(
                            config=config,
                    ...
                    LOGGER.info("Finished download for chat %s", chat_url)
                except Exception as exc:
                    failures.append(chat_url)
                    LOGGER.exception("Download failed for %s: %s", chat_url, exc)
            if failures:
                _set_status(f"Finished with errors ({len(failures)} failed)", "orange")
            else:
    ...
    def _on_start() -> None:
        ...
        chat_urls = _collect_chat_urls(chat_text)
        output_root = output_entry.get().strip()
        if not chat_urls or not output_root:
            LOGGER.error("Chat URLs and output root are required.")
            _set_status("Provide chat list and output root", "red")
            return
        ...
        _set_status("Preparing download", "blue")
        download_thread = threading.Thread(
            target=_worker,
            args=(config, chat_urls, date_from_entry.get(), date_to_entry.get(), selected_types),
            daemon=True,
        )
