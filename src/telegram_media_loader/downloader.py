...
from typing import AsyncIterator, Callable, Iterable, Optional, Tuple
...
def _parse_chat_url_and_topic(chat_url: str) -> tuple[str | int, Optional[int]]:
    """
    Returns a tuple of (chat reference, topic_id).

    Supports web URLs like https://web.telegram.org/a/#-1001234567890_6
    where the suffix after '_' is the forum topic id.
    """
    value = chat_url.strip()
    topic_id: Optional[int] = None

    topic_match = re.search(r"#(-?\d+)_([0-9]+)", value)
    if topic_match:
        chat_ref_raw = topic_match.group(1)
        topic_id = int(topic_match.group(2))
        chat_ref = int(chat_ref_raw) if chat_ref_raw.lstrip("-").isdigit() else chat_ref_raw
        return chat_ref, topic_id

    return _parse_chat_url(value), topic_id


class ProgressReporter:
    def history_started(self, total_messages: Optional[int]) -> None:
        ...
    ...
    def download_started(self, label: str, total: Optional[int]) -> Optional[Callable[[int, int], None]]:
        ...
...
class MediaDownloader:
    def __init__(
        ...
        *,
        ...
        self._entity = None
        # (topic_id, topic_title, top_message_id)
        self._topic_override: Optional[Tuple[int, Optional[str], Optional[int]]] = None
        self._topic_filter_id: Optional[int] = None
    ...
    async def download(
        ...
        chat_ref, topic_id = _parse_chat_url_and_topic(chat_url)
        ...
        total_messages = await self._history_count(entity)
        ...
        if topic_id:
            LOGGER.info("Limiting to forum topic id %s", topic_id)
        if topics:
            for topic in topics:
                ...
                if topic_id and tid == topic_id:
                    ...
                    top_msg = getattr(topic, "top_message", None)
                    self._topic_override = (topic_id, topic_title, top_msg)

        # If topic was specified but not found in the first page, try to fetch it directly.
        if topic_id and not self._topic_override:
            fetched = await self._fetch_topic_by_id(entity, topic_id)
            if fetched:
                self._topic_override = fetched
            else:
                self._topic_override = (topic_id, self._topic_cache.get(topic_id), None)
        self._topic_filter_id = topic_id

        # We iterate full history and filter by topic_id in _handle_message to avoid TOPIC_ID_INVALID.
        reply_to_target = None

        if self._progress:
            self._progress.history_started(total_messages)

        iterator = self._iter_messages(entity, date_to)
        await self._process_iterator(
            iterator,
            chat_slug,
            chat_type,
            allowed_media,
    ...
    async def _process_iterator(
        ...
        async for message in iterator:
            if self._progress:
                self._progress.message_processed()
    ...
    async def _handle_message(
        ...
        chat_id = getattr(self._entity, "id", None) or 0
        media_type, mime_type = self._describe_media(message)
        if media_type not in allowed_media:
            return False
        if self._topic_filter_id is not None:
            msg_topic_id, _ = await self._resolve_topic_info(message, None)
            if msg_topic_id != self._topic_filter_id:
                return False
        if self._db.already_downloaded(chat_id, message.id, 1):
            LOGGER.debug("Skipping already downloaded message %s (chat %s)", message.id, chat_id)
            return False
    ...
    async def _process_media(
        ...
        topic_id, topic_title = await self._resolve_topic_info(message, self._topic_override)
        ...
        for attempt in range(2):
            progress_callback = (
                self._progress.download_started(filename, self._media_size_hint(message))
                if self._progress
                else None
            )
            try:
                saved = await message.download_media(file=str(destination), progress_callback=progress_callback)
                if saved:
                ...
                if self._progress:
                    self._progress.download_finished()
    ...
    async def _resolve_topic_info(
        self,
        message: Message,
        topic_override: Optional[Tuple[int, Optional[str], Optional[int]]],
    ) -> Tuple[Optional[int], Optional[str]]:
        override_id, override_title, _ = topic_override or (None, None, None)
        topic_id: Optional[int] = getattr(message, "topic_id", None)

        if topic_id is None:
            topic_id = self._topic_id_from_reply(message)

        # If message lacks topic info, fall back to override (e.g., specified via URL)
        if topic_id is None:
            topic_id = override_id
        ...
        if topic_id is not None:
            ...
            if not topic_title and override_id == topic_id and override_title:
                topic_title = override_title
    ...
    async def _fetch_topic_by_id(
        ...
        try:
            req = GetByID(**{arg_name: entity}, topics=[topic_id])
            response = await self._client(req)
            topics = getattr(response, "topics", []) or []
            if not topics:
                return None
            ...
            if title:
                self._topic_cache[topic_id] = title
            return (topic_id, title, top_message)
        except Exception:
            LOGGER.debug("Unable to fetch topic id %s directly", topic_id, exc_info=True)
            return None
    ...
    async def _history_count(self, entity) -> Optional[int]:
        try:
        ...
        try:
            ...
            LOGGER.debug("Unable to fetch history count", exc_info=True)
            return None

    def _media_size_hint(self, message: Message) -> Optional[int]:
