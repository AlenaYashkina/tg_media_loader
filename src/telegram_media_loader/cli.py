...
async def run_download(
    *,
    ...
    async with TelethonClientManager(config) as client:
        with DownloadDB(config.sqlite_path) as db:
...
class TqdmProgress(ProgressReporter):
    def __init__(self) -> None:
        self._messages: Optional[tqdm] = None
        self._media = tqdm(desc="Downloaded", unit="media", position=1)
    ...
    def message_processed(self) -> None:
        if not self._messages:
            self._messages = tqdm(desc="Messages", unit="msg", position=0)
    ...
    def download_started(self, label: str, total: Optional[int]) -> Optional[Callable[[int, int], None]]:
        # Skip per-file bars; focus on chat-level progress.
        return None
